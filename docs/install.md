...
С версии 0.6.0 требуется перед запуском daemonctrl.php включить необходимые демоны. Это можно сделать через меню "Настройки" в разделе "Модули". По умолчанию все демоны модулей отключены.
