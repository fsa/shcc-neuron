...
Устройство поддерживает стандартные [методы интерфейса Smarthome\DeviceInterface](../deviceinterafce.md), которые необходимы, в основном, для работы системы.
...
## getActions()
УСТАРЕЛО. Возвращает список изменений состояния лампы после обновления данных и очищает его. Из-за особенностей протокола обмены с лампой подлежит удалению.
...
## sendSetCtAbx(int $ct_value,int $duratin=0): int
...
## sendStartCF(int $count,int $action,string $flow_expression): int
