# Система "умный дом"
Проект на стадии разработки. В процессе возможны значительные измнения в структуре базы данных, при этом не создаются скрипты миграции. Эти изменения необходимо производить вручную или пересоздавать базу данных.
...
- сервер баз данных MariaDB-10.1 и выше (возможно использовать MySQL, но его работа не тестируется).

Версии ПО выбираются те, которые доступны в Ubuntu 18.04 LTS и новее без необходимости подключать сторонние репозитории. Возможна установка на одноплатные компьютеры Raspbian Pi 2/3 и др., которые поддерживаются Ubuntu или Raspbian в которой используются необходимые репозитории для обновления PHP до требуемой версии.
...
- веб-интерфейс;
- демоны для работы с протоколами устройств (имеется шаблон скрипта запуска для systemd и шаблон скрипта для классической системы инициализации);
...
В примере пользовательского крипта выполняются следующие задачи:
