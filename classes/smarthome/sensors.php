...
class Sensors {
    ...
    public static function getDeviceMeters($module,$uid) {
        $stmt=DB::prepare('SELECT m.id, m.property, d.place_id, m.meter_unit_id FROM meters m LEFT JOIN devices d ON m.device_id=d.id LEFT JOIN modules md ON d.module_id=md.id WHERE md.name=? AND d.uid=?');
