...
class Sensors
{
    ...
    {
        $s = App::sql()->prepare('SELECT id, uid, history FROM sensors WHERE device_property=?');
        ...
        $s = App::sql()->prepare('INSERT INTO ' . $sensor->history . ' (sensor_id, value, timestamp) VALUES (?, ?, ?)');
        $datetime = date('c', $ts);
    ...
    {
        $s = App::sql()->prepare('SELECT * FROM sensors WHERE uid=?');
        $s->execute([$uid]);
        $sensor = $s->fetchObject();
        ...
        $stmt = App::sql()->prepare('SELECT ROUND(EXTRACT(EPOCH FROM timestamp)*1000) AS ts,value FROM ' . $sensor->history . ' WHERE sensor_id=:sensor_id' . $period . ' ORDER BY timestamp');
        $stmt->execute($params);
    ...
    public static function getHistoryJson(string $uid, $from = null, $to = null)
    {
        ...
        $stmt = App::sql()->prepare('SELECT json_build_object(\'name\', \'' . $sensor->description . '\',\'unit\', \'' . self::PROPERTIES[$sensor->property][1] . '\',\'data\', (SELECT array_agg(array[ts, value]) FROM (SELECT ROUND(EXTRACT(EPOCH FROM timestamp)*1000) AS ts, value FROM ' . $sensor->history . ' WHERE sensor_id=:sensor_id' . $period . ' ORDER BY timestamp) AS data))');
        $stmt->execute($params);
    ...
    {
        $s = App::sql()->query('SELECT * FROM sensors ORDER BY uid');
